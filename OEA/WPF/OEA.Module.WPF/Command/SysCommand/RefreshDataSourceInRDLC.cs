...
{
    [Command(Label = "刷新 RDLC 字段", ToolTip = "根据当前数据生成或刷新 RDLC 文件中的数据源", GroupType = CommandGroupType.System)]
    public class RefreshDataSourceInRDLC : ClientCommand<ReportObjectView>
    {
        #region 常量定义

        public static readonly string ReportDataSouceName = "OEA";
...
";
        ...
        {
            var btn = App.MessageBox.Show("确定生成或刷新当前报表的数据源？", MessageBoxButton.YesNo);
            ...
            try
            {
                ...
                var res = App.MessageBox.Show(@"刷新数据源成功，是否拷贝路径到剪贴板（方便在设计器中直接打开）：" + reportPath, MessageBoxButton.YesNo);
        ...
        {
            foreach (var property in meta.EntityProperties)
            {
        ...
        private static string FormatPropertyLabel(EntityPropertyViewMeta property)
        {
