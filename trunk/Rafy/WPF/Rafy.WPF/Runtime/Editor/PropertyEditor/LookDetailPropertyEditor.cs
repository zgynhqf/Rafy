...
namespace Rafy.WPF.Editors
{
    ...
    /// </summary>
    public class LookDetailPropertyEditor : PropertyEditor
    {
        ...
        private void PopupDetail()
        {
            ...
            //弹出窗体显示详细面板
            var viewMeta = UIModel.Views.Create(currType) as WPFEntityViewMeta;
            viewMeta.NotAllowEdit = true;

            var detailView = AutoUI.ViewFactory.CreateDetailView(viewMeta);
            detailView.Data = tmp;
            detailView.Control.VerticalAlignment = VerticalAlignment.Top;

            App.Windows.ShowDialog(detailView.Control, w =>
            {
                w.Title = "详细信息".Translate();
