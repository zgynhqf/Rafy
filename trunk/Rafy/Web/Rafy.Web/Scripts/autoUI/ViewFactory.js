...
Ext.define('Rafy.autoUI.ViewFactory', {
    ...
    createListView: function (block) {
        var view = new Rafy.view.ListView(block);
    ...
    _createTreeListControl: function (gridConfig, store, view) {
        ...
        var c = Rafy.findFirst(gridConfig.columns, function (c) { return c.editor != null; });
    ...
    createConditionView: function (block) {
        var view = new Rafy.view.ConditionView(block);

        var formCfg = Ext.merge({}, block.formConfig);
        if (!formCfg.tbar) formCfg.tbar = [];
        formCfg.tbar.push({ "command": "Rafy.cmd.ExecuteQuery" });
    ...
    createDetailView: function (block, entity) {
        var view = new Rafy.view.DetailView(block);
    ...
    _createCommands: function (panelCfg, view) {
        if (panelCfg.tbar) {
            var me = this;
            Rafy.each(panelCfg.tbar, function (ti) { me._setHandler(ti, view); });
        }
    },
