...
namespace Rafy.Domain
{
    ...
    /// DbTable 查找器。
    /// </summary>
    internal static class DbTableFinder
    {
        /// <summary>
        /// 获取该实体对应的 ORM 运行时对象。
        /// 
        ...
        /// <returns></returns>
        internal static DbTable TableFor(Type entityType)
        {
            return RepositoryFactoryHost.Factory.FindByEntity(entityType).RdbDataProvider.DbTable;
        }

        internal static TextWriter AppendQuote(this TextWriter sql, DbTable table, string identifier)
        {
            table.AppendQuote(sql, identifier);
            return sql;
        }

        internal static TextWriter AppendQuoteName(this TextWriter sql, DbTable table)
        {
