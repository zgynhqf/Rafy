...
namespace OEA.Module.WPF.Controls
{
    ...
    public partial class TreeGrid
    {
        ...
        private void RebindRoots()
        {
            ...
            #region 从数据源中查找出所有的根结点。

            if (this._itemsSource != null)
            {
                //所有的树型结点的 Id 的缓存。
                var idsCache = new Lazy<List<int>>(
                    () => this._itemsSource.Cast<object>().Select(e => GetId(e)).ToList()
                    );

                //如果某个 item 是个 rootItem，则把它加入到 roots 中
                foreach (object item in this._itemsSource)
                {
            ...
            #region 表格模式下，重新绑定数据时，如果没有分组，则打开 UIV。
        ...
        protected override sealed TreeViewItem EnsureNodeIsVisible(object item)
        {
        ...
        /// </returns>
        protected internal virtual GridTreeViewRow FindOrGenerateNode(object item)
        {
        ...
        private bool IsRootItem(object item, Lazy<List<int>> allIdsCache)
        {
            ...
            if (this._rootPId != null)
            {
                ...
                if (this._rootPId == GetPId(item)) { return true; }
            }
            else
            {
                //未进行过滤时，如果是 根对象 或者 列表中不包含该对象的父对象，都可以直接加入根对象集合中。
                var pid = GetPId(item);
                if (pid == null || !allIdsCache.Value.Contains(pid.Value)) { return true; }
            }
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        public GridTreeViewRow FindRow(object item)
        {
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        public GridTreeViewRow GetRow(object item)
        {
            var row = this.FindRow(item);
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        private int GetId(GridTreeViewRow item)
        {
