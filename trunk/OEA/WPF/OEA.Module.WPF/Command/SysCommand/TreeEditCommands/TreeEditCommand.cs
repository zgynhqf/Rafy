...
namespace OEA.WPF.Command
{
    ...
    public class LevelUpCommand : EditOrderedTreeListViewCommand
    {
        public override bool CanExecute(ListObjectView view)
        {
            return base.CanExecute(view) &&
                view.Current != null && view.Current.TreeParent != null;
        }

        public override void Execute(ListObjectView view)
        {
            var current = view.Current;
            var pp = current.TreeParent.TreeParent;
            ...
            if (pp == null)
            {
                ...
                var roots = list.FindRoots().ToList();
                var parentRootIndex = roots.IndexOf(current.TreeParent);

                current.TreeParent = pp;

                //如果已经移动到最高层，因为 TreeParent = null 会把该节点从列表中删除，
                //所以应该把这个节点再次加入到根列表中。
                var index = parentRootIndex == roots.Count - 1 ?
                    list.Count : list.IndexOf(roots[parentRootIndex + 1]);
    ...
    public class LevelDownCommand : EditOrderedTreeListViewCommand
    {
        ...
        {
            IList<Entity> parentList = node.TreeParent != null ?
                node.TreeParent.TreeChildren : node.ParentList.FindRoots().ToArray() as IList<Entity>;
