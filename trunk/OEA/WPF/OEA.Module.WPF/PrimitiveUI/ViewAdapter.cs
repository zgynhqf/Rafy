...
namespace OEA.Module.WPF
{
    ...
    internal static class ViewAdapter
    {
        ...
        /// 
        /// ObjectView.IsVisible 和 TabItem.Visibility 的关系：
        /// 在目标上，两者实现双向绑定。
        /// 1. ObjectView.IsVisible 的改变，直接通过 IsVisibleChanged 事件的处理函数来设置对应的 TabItem 的 Visibility 属性。
        /// 2. TabItem.Visibility 先使用 OnWay 的 Binding 来直接绑定在实体类父对象的某个属性上，实现动态可见性。
        ///     然后，在 Binding 的 UpdateTarget 事件处理函数中，再设置 ObjectView.IsVisible 的值。
        ///     代码位于 ListObjectView.ResetVisibility() 方法中。
        /// </summary>
        /// <param name="childView"></param>
        /// <param name="control"></param>
        public static void AdaptView(ObjectView childView, TabItem control)
        {
            //IsActive
        ...
        public static void AdaptView(ObjectView parentView, TabControl childrenTab)
        {
            ...
            childrenTab.SelectionChanged += (sender, e) =>
            {
                ...
                if (sender == e.OriginalSource && e.AddedItems.Count > 0)
                {
                    ...
                    {
                        var childView = WPFMeta.GetObjectView(item);
