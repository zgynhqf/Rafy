...
namespace OEA.MetaModel.XmlConfig.Web
{
    /// <summary>
    /// UI 块的配置
    /// </summary>
    public class BlockConfig : MetaConfig
    {
        ...
        {
            if (PageSize != null) evm.PageSize = PageSize.Value;
            if (GroupBy != null)
            {
            ...
            if (OEAEnvironment.IsWeb)
            {
                foreach (var cmd in this.Commands)
                {
                    if (cmd.IsChanged())
                    {
                        var jsCmd = evm.WebCommands.Find(cmd.Name);
            ...
            else
            {
                foreach (var cmd in this.Commands)
                {
                    if (cmd.IsChanged())
                    {
                        var wpfCmd = evm.WPFCommands.Find(cmd.Name);
