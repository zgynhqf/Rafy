...
namespace OEA.Library.Caching
{
    ...
    internal class EntityListVersionRepository : IEntityListVersionRepository
    {
        ...
        public void Clear()
        {
            var cacheRepo = RF.Create<ScopeVersion>();

            var allList = cacheRepo.GetAll(false);
            allList.Clear();
        ...
        public void UpdateVersion(Type region, Type scopeClass, string scopeId)
        {
            ...
            //if (!OEAEnvironment.Location.IsOnServer()) throw new InvalidOperationException("!CslaEnvironment.IsOnServer must be false.");

            if (BeginBillSaveTran.Instance.IsStarted)
            {
                BeginBillSaveTran.Instance.UpdateVersion(region, scopeClass, scopeId);
            }
            else
            {
                //此操作使用异步执行，原因：
        ...
        /// </summary>
        /// <returns></returns>
        public IDisposable BeginBillSave()
        {
            BeginBillSaveTran.Instance.EndBillSave();
            return BeginBillSaveTran.Instance;
        }

        public void EndBillSave()
        {
            BeginBillSaveTran.Instance.EndBillSave();
        }

        private class BeginBillSaveTran : IDisposable
        {
            public static readonly BeginBillSaveTran Instance = new BeginBillSaveTran();

            private List<SaveItem> _items = new List<SaveItem>();

            private BeginBillSaveTran() { }

            public bool IsStarted;

            public void BeginBillSave()
            {
                this.IsStarted = true;
            }
        ...
        private class ScopeVersionListCache
        {
            ...
            public ScopeVersion Find(string classRegion, string scopeClassName, string scopeId)
            {
                ...
                if (this._regionValues == null || this.IsExpired())
                {
                    lock (this)
                    {
                        ...
                        {
                            var allValues = RF.Concreate<ScopeVersionRepository>().GetAll() as ScopeVersionList;
                            this.SetAllValues(allValues);
                        }
                        //如果过期，则差异更新。
                        else if (this.IsExpired())
                        {
                            var values = RF.Concreate<ScopeVersionRepository>().GetList(_serverTime);
            ...
            {
                return (DateTime.Now - this._localTime).TotalSeconds > ExpiredSeconds;
            }
        }
