...
namespace Rafy.Domain.ORM
{
    ...
    internal abstract class DbTable : ITable
    {
        ...
        internal List<DbColumn> _columns;

        private DbColumn _identityColumn;

        private DbColumn _pkColumn;
        ...
        internal int Update(IDbAccesser dba, Entity item)
        {
            ...
            List<DbColumn> lobColumns = null;

            if (_hasLOB)
            {
                lobColumns = new List<DbColumn>();
                ...
                for (int i = 0, c = _columns.Count; i < c; i++)
                {
                    ...
                    if (column.IsLOB && item.FieldExists(column.Property))
                    {
            ...
            {
                var column = _columns[i];
                if (!column.IsPrimaryKey && !column.IsLOB)
                {
                    var value = column.ReadParameterValue(item);
                    parameters.Add(value);
                }
        ...
        private string GenerateUpdateSQL(IList<DbColumn> lobColumns)
        {
            var sql = new StringWriter();
            sql.Write("UPDATE ");
            sql.AppendQuoteName(this);
        ...
        protected bool AutoSelectionForLOB(IQuery query)
        {
            if (query.Selection == null && !query.IsCounting && _hasLOB)
            {
                ...
                var columns = allColumns.Where(n => !(n as ColumnNode).DbColumn.IsLOB);
        ...
        internal DbColumn FindByPropertyName(string propertyName)
        {
            ...
            foreach (DbColumn column in _columns)
            {
                if (column.Property.Name == target) return column;
            }
