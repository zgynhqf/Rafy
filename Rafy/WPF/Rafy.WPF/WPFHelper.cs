...
namespace Rafy.WPF
{
    internal static class WPFHelper
    {
        ...
        internal static void SetTrackFocusScope(DependencyObject scope)
        {
            scope.SetIfNonLocal(FocusTrackerAdorner.TrackFocusScopeProperty, TrackFocusScope.Scope);
            scope.SetIfNonLocal(FocusTrackerAdorner.AwareInputElementsProperty, new Type[]{
                typeof(TextBox), typeof(PasswordBox), typeof(ComboBox), typeof(DatePicker),
                typeof(DateTimeInputControl),
            });
        }
    }
}
