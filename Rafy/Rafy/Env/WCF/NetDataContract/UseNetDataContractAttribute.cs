...
namespace Rafy.WCF
{
    ...
    /// 反之，如果使用后者进行序列化，那么必须使用 KnownTypes 标记在接口上标记它可序列化的所有类型，使得 IWcfPortal 将不能作为一个通用的接口程序。
    /// 
    /// 但是，使用了这个标记之后，也会带来相应的不利的影响：由于使用 .NET 内部的序列化规范，这个接口将不再满足 WSDL 的规范，使得其它平台无法调用此接口，也导致了无法为 .NET 的项目直接使用服务的地址来添加服务的引用。
    /// 
    /// 其它区别：
    /// https://stackoverflow.com/questions/7890693/netdatacontractserializer-vs-datacontractserializer
    /// 
